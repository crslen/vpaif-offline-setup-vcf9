...
####
# export PRIVATE_REGISTRY="rp-sm-n-ag-harbor.pse.lab" 
# export 
####
set -u
...
GPU_OPERATOR_VERSION=${GPU_OPERATOR_VERSION:-"25.10.1"}
HTTP_SERVER="http://172.30.132.75"

REGISTRY_SECRET_NAME=${REGISTRY_SECRET_NAME:-"ngc-secret"}
PRIVATE_REGISTRY=${PRIVATE_REGISTRY:-"rp-r169-env1-harbor.pse.lab"}
...
create_operator_namespace() {
    ...
    printf "deb [arch=amd64] ${HTTP_SERVER}/ jammy main universe\ndeb [arch=amd64] ${HTTP_SERVER}/ jammy-updates main universe\ndeb [arch=amd64] ${HTTP_SERVER}/ jammy-security main universe\n" > custom-repo.list
...
install_operator() {
    ...
    helm upgrade --install --wait --insecure-skip-tls-verify gpu-operator gpu-operator-v${GPU_OPERATOR_VERSION}.tgz -n ${NAMESPACE} \
        ...
        --set driver.image="vgpu-guest-driver-7" \
        ...
        --set node-feature-discovery.image.tag="v0.18.3" \
        --debug \
        ${HELM_INSTALL_OPTS}
...
cleanup_operator() {
    ...
    kubectl delete repo-config -n ${NAMESPACE}
}
